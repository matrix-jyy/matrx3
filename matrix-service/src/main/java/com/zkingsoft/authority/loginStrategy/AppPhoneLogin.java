...
public class AppPhoneLogin implements LoginStrategy {

	private SysUsersService sysUsersService;
	private SysUsers user;
	

	public AppPhoneLogin(SysUsers user, SysUsersService sysUsersService) {
		this.user=user;
	...
	public Object login() {
		// 是否同时有账号和手机号码
		if (user.getSuTel() != null && user.getSuPassword() != null) {
			// 加密密码
			try {
			...
			userQuery.setSuTel(user.getSuTel());
		...
		throw new GlobleException("请输入手机号和密码");
		}
		return null;
	}
